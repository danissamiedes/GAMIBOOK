...
/**
 * One vendor: who they are, and everything written down about them.
 *
 ...
 *
 * Scoped to REGULAR, so the two kinds of Vendor cannot be read through each
...
export default async function VendorPage({
  ...
  const party = await prisma.vendor.findFirst({
    where: { id, kind: "REGULAR", ...scope.where },
  });
  if (!party) notFound();
  ...
  return (
    <>
      <PageHeader
        title={party.name}
        description={`Vendor · ${party.defaultCurrency} · ${party.paymentTermsDays} day terms${
          party.isActive ? "" : " · inactive"
        }`}
      />

      <div className="mb-4 flex gap-2">
        <Link href={`/vendors?edit=${party.id}`}>
